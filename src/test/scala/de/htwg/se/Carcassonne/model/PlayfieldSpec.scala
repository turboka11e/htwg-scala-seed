  ...
  "A Playfield" when {
    ...
    "new with field size 3 set" should {
      ...
      "allow to add player" in {
        p2.addPlayer("Test") should be(Playfield(grid = new Grid(3), gameState = 2, players = Nil ::: List(Player("Test"))))
      }
    }
    "new with field size 3 and one player added" should {
      val p3 = Playfield().fieldSize(3).addPlayer("Test")
      "return fresh Card" in {
        p3.getFreshCard(0) should be(Playfield(grid = new Grid(3),
          gameState = 2, players = Nil ::: List(Player("Test")), freshCard = CardCreator(0).randCard(0)))
      }
    }
    "new with field size 3 and one player addes and fresh Card" should {
      ...
      "rotate freshCard to right" in {
        p4.rotateR should be(Playfield(grid = new Grid(3),
          gameState = 2, players = Nil ::: List(Player("Test")), freshCard = CardCreator(0).randCard(0).rotateRight))
      }
      "rotate freshcard to left" in {
        p4.rotateL should be(Playfield(grid = new Grid(3),
          gameState = 2, players = Nil ::: List(Player("Test")), freshCard = CardCreator(0).randCard(0).rotateLeft))
      }
      "allow selections of an area" in {
        p4.selectArea(0) should be(Playfield(grid = new Grid(3),
