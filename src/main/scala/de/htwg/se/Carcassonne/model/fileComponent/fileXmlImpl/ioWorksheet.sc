...
def cardToXml(row:Int, col:Int):Elem = {
  ...
  <card row={row.toString} col={col.toString} name={card.id._1.toString} rotation={card.id._2.toString}>
    {if (indexArea != -1) {
      <selectedArea index={indexArea.toString} player={card.areas(indexArea).player.toString}></selectedArea>
  }}
  </card>
}

val xml = <playfield size={playfield.grid.size.toString} isOn={playfield.isOn.toString} freshCard={playfield.freshCard.card.id._1.toString} gameState={playfield.getGameState.toString}>
  <players>
    {for {
